...
function criaListagemTabela(jsonDados) {
  ...
  $.each(jsonDados, function (indice, contato) {
    ...
    tabela +=
      '<td><a href="javascript:void(0);" id="confirma_excluir" contato_id="' +
      contato.id +
...
function getFormularioInclusao() {
  ...
  form +=
    '<label for="nome" class="col-sm-2 control-label glyphicon glyphicon-user"></label>';
  ...
  form += '<div class="form-group">';
  form +=
    '<label for="telefone" class="col-sm-2 control-label glyphicon glyphicon-phone"></label>';
  form += '<div class="col-sm-8">';
  form +=
  ...
  form += '<div class="form-group">';
  form +=
    '<label for="endereco" class="col-sm-2 control-label glyphicon glyphicon-home"></label>';
  form += '<div class="col-sm-8">';
  form +=
    '<textarea class="form-control" rows="3" id="endereco" placeholder="Endereço"></textarea>';
  form += "</div>";
  form += "</div>";
  form += '<div class="form-group">';
  form +=
    '<label for="text" class="col-sm-2 control-label glyphicon glyphicon-signal"></label>';
  form += '<div class="col-sm-8">';
  form +=
  ...
  form +=
    '<button type="button" id="salvar_contato" class="btn btn-primary">Salvar</button>';
  form += "</div>";
  form += "</div>";
  form += "</form>";
...
function getFormularioEdicao(jsonDado) {
  ...
  $.each(jsonDado, function (indice, contato) {
    ...
    form += '<div class="form-group">';
    form +=
      '<label for="telefone" class="col-sm-2 control-label glyphicon glyphiconphone"></label>';
    form += '<div class="col-sm-8">';
    form +=
      '<input type="tel" class="form-control" id="telefone" placeholder="Telefone" value="' +
      contato.telefone +
      '">';
    form += "</div>";
    form += "</div>";
    form += '<div class="form-group">';
    form +=
      '<label for="email" class="col-sm-2 control-label glyphicon glyphiconenvelope"></label>';
    form += '<div class="col-sm-8">';
    form +=
    ...
    form += '<div class="col-sm-8">';
    form +=
      '<textarea class="form-control" rows="3" id="endereco" placeholder="Endereço">' +
      contato.endereco +
      "</textarea>";
    form += "</div>";
    form += "</div>";
...
function validaDados() {
  if ($("#nome").val().trim().length == 0) {
    $("#nome")
      .addClass("error")
      .after('<span class="error">Por favor, preencha esse campo.</span>');
    return false;
  }
  return true;
}
